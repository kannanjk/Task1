...
genericSortIcons = () => {
  ...
  const fieldName = Session.get('field');
 ...
 */
getSortOptions = () => {
  const fieldName = $(event.target).attr('id');
  const order = (Session.get('field') !== fieldName) ? null : Session.get('order');
  const sortOrder = order && order === 1 ? -1 : 1;
